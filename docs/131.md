...
由Apache Software Foundation提供的**和 **N** 的 **T** 工具是用 **Java** 编写的**构建工具**。 它已经很老了（Maven在这个领域赢得了胜利，Gradle在相处中），非常受欢迎，并且在大多数项目中仍然可以找到。 Apache ANT的优点在于，它仅执行被告知要做的事情。 引擎盖下没有太多魔术发生。 *（请注意，有时这可能是一把双刃剑！）***

创建可执行文件意味着

*   在src文件夹下编译所需的.java文件
*   为JAR创建清单文件
