...
在这三个注释下的**方法**是*公开的*，返回类型为 *void* 。 在 *junit.framework.TestCase* 类下，我们有 **setUp（）**和 **tearDown（）**方法。 最佳实践是重写这些方法，以分别编写初始化代码和清除代码。 这不仅可以防止内存泄漏，还可以使代码更易于阅读。 JUnit 首先调用 setUp（）方法，然后调用 test 方法，最后调用 tearDown（）方法。 对于附加到@Test 的每个测试方法，都会发生这种情况。
