...
*   返回类型必须与在Superclass中的重写方法中声明的返回类型相同或为该子类型的子类型。
...
如果要在执行子类方法之前调用超类的重写方法，该怎么办。 您可以使用SUPER关键字。
...
##### Java中的[接口](https://javabeginnerstutorial.com/core-java-tutorial/java-interface/ "Interface in java")